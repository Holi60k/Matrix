...
// 	Második nummod.... ez is volt
//  Harmadik nummod beadandó
//  Elkészítve: 2015.11.26
#include <iostream>
#include <limits>
#include <cmath>
#include <fstream>
...
Vector<double> szamolas(Vector<double> y,double lam, Vector<double> x) {
	Vector<double> temp(y.GetX());
	for(int i = 0; i < temp.GetX(); i++) {
		temp.FillVector(y.GetValue(i) - (lam*x.GetValue(i)),i);
	}
	return temp;
}
...
int main() {
    ...
    for(int l = 0; l < N; l++) {
    	...
    	if(y0.isZeroVector()) {
	    	...
	    	x = mSzv(A,y);
	    	k = 0;
	    	lambda.push_back(BelsoSz(&y,&x));
	    	lambdaNull = lambda[0];
	    	for(k = 1; k < mI; k++) {

	    		y = x.Normalize();
	    		x = mSzv(A,y);
	    		//std::cout << BelsoSz(&y,&x) << std::endl;
	    		lambda.push_back(BelsoSz(&y,&x));
	    		//std::cout << "lamdba[" << m << "]" << lambda[m] << std::endl;

	    		if(std::fabs(lambda[k]-lambda[k-1]) <= Eps*(1+std::fabs(lambdaNull))) {
	    			//std::cout << "siker: "  << kettoNorma(szamolas(x,lamba[m],y)) << std::endl;
		   			if(kettoNorma(szamolas(x,lambda[k],y)) <= Eps) {
	    				std::cout << "siker " << std::fixed << lambda[k] << " " << y << k << " "<<std::endl;
	    			} else {
	    				std::cout << "sikertelen " << std::fixed << lambda[k] << " " << y << k << " " <<std::endl;
	    			}
	    			break;
	    		}
	    	}
